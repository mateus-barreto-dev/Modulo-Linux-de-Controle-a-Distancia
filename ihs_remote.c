...
static int video_stream_thread(void *data) {
    ...
    while (!kthread_should_stop()) {
        ...
        int bpp = fb->format ? fb->format->cpp[0] : 4; 
        size_t tamanho_envio = real_width * real_height * bpp;
        ...
        if (ret_vmap == 0) {
            ...
            if (pixel_ptr) {
                ...
                for (y = 0; y < real_height; y++) {
                    memcpy(buffer_envio + (y * real_width * bpp), 
                           pixel_ptr + (y * real_pitch), 
                           real_width * bpp);
                }
            }
            ...
            if (pixel_ptr) {
                u32 header[3] = { (u32)real_width, (u32)real_height, (u32)bpp };
                struct msghdr msg;
                struct kvec vec;
...
finalizar_thread:
    ...
    // Libera o socket do cliente de vídeo que acabou de desconectar
